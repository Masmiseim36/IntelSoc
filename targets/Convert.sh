...
for file in *; do 
	if [ -f "$file" ]; then 
		...
		# For Cortex M4 / M7
		echo "C:\Program Files\Rowley CrossWorks for ARM 4.8\bin\svd2mm.exe" -tcf $file $filename$extension
		"C:\Program Files\Rowley CrossWorks for ARM 4.8\bin\svd2mm.exe" -tcf $file $filename$extension
